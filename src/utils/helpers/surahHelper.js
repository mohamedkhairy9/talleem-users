 ...
 */
export async function loadSurahData() {
 ...
 */
export async function getSurahData(surahNumber, surahData) {
 ...
 */
export async function getSurahNameArabic(surahNumber, surahData) {
 ...
 */
export async function getSurahNameEnglish(surahNumber, surahData) {
    const surah = await getSurahData(surahNumber, surahData);
 ...
 */
export async function getSurahName(surahNumber, surahData) {
    const surah = await getSurahData(surahNumber, surahData);
    return surah?.name || '';
}
/**
 ...
 */
export function getVerseKeyDisplay(verseKey, surahData, lang) {
    if (!verseKey?.trim() || !surahData)
        return null;
 ...
 */
export async function getSurahDisplayName(surahNumber, locale = 'ar', surahData) {
    if (locale === 'ar') {
 ...
 */
export async function getSurahGlyph(surahNumber, surahData) {
    const surah = await getSurahData(surahNumber, surahData);
    return surah?.glyph || '';
}
/**
 ...
 */
export async function getSurahInfo(surahNumber, locale = 'ar', surahData) {
    const surah = await getSurahData(surahNumber, surahData);
    if (!surah)
        return null;
    return {
 ...
 */
export function clearSurahDataCache() {
    surahDataCache = null;
 ...
 */
export function getJuzFirstVerseKey(juzNumber) {
 ...
 */
export function getJuzNumberForVerseKey(verseKey) {
    ...
    for (let j = 1; j <= 30; j++) {
        ...
        if (!first)
            continue;
        if (compareVerseKeys(key, first) < 0)
            return Math.max(1, j - 1);
        if (j === 30)
            return 30;
        const nextFirst = getJuzFirstVerseKey(j + 1);
        if (nextFirst && compareVerseKeys(key, nextFirst) < 0)
            return j;
    }
    return 30;
}
 ...
 */
function verseBeforeVerseKey(verseKey) {
 ...
 */
export function getJuzLastVerseKey(juzNumber) {
    if (juzNumber >= 30)
        return '114:6';
 ...
 */
export function getSurahNumbersInJuz(juzNumber) {
    if (juzNumber < 1 || juzNumber > 30)
        return [];
    ...
    for (let s = 1; s <= 114; s++) {
        ...
        if (compareVerseKeys(first, endKey) <= 0 && compareVerseKeys(last, startKey) >= 0)
            surahs.push(s);
    }
    return surahs;
}
 ...
 */
export function getSurahFirstVerseKey(surahId) {
    return `${surahId}:1`;
}
/**
...
export const VERSE_COUNT_PER_SURAH_READONLY = VERSE_COUNT_PER_SURAH;
/** Whether (surah, ayah) is the last verse of that surah (long verses often need an extra page in 604 layout). */
export function isLastVerseOfSurah(surah, ayah) {
 ...
 */
export function getVerseKeyFromGlobalId(verseId) {
    if (verseId < 1)
        return '1:1';
 ...
 */
export function verseKeysBetween(startKey, endKey, clipStart, clipEnd) {
    ...
    for (let s = startSurah; s <= endSurah; s++) {
        ...
        for (let a = firstAyah; a <= lastAyah; a++) {
            const key = `${s}:${a}`;
            if (clipStart && compareVerseKeys(key, clipStart) < 0)
                continue;
            if (clipEnd && compareVerseKeys(key, clipEnd) > 0)
                continue;
            keys.push(key);
        }
    }
...
let mushafPagesCache = null;
let mushafPagesPromise = null;
/**
 ...
 */
export function loadMushafPages() {
    if (mushafPagesCache && mushafPagesCache.length === 604)
        return Promise.resolve(mushafPagesCache);
    if (mushafPagesPromise)
        return mushafPagesPromise;
    mushafPagesPromise = fetch('/data/mushaf_pages.json')
        .then((r) => {
        if (!r.ok)
            throw new Error(`mushaf_pages: ${r.status}`);
        return r.json();
    })
        .then((data) => {
 ...
 */
export function getPageForVerseKey(verseKey, pages) {
    const key = verseKey.trim();
    if (!pages.length)
        return 1;
    if (compareVerseKeys(key, pages[0].start_verse_key) < 0)
        return 1;
    if (compareVerseKeys(key, pages[pages.length - 1].end_verse_key) > 0)
        return 604;
    ...
    while (lo < hi) {
        const mid = (lo + hi + 1) >> 1;
        if (compareVerseKeys(pages[mid].start_verse_key, key) <= 0)
            lo = mid;
        else
            hi = mid - 1;
    }
    const entry = pages[lo];
    if (compareVerseKeys(key, entry.end_verse_key) <= 0)
        return entry.page;
    return lo + 1 < pages.length ? pages[lo + 1].page : 604;
}
let juzPagesCache = null;
let juzPagesPromise = null;
/**
 * Load juz page ranges from /data/juz_pages.json.
 * Cached after first load.
 */
export function loadJuzPages() {
    if (juzPagesCache && juzPagesCache.length === 30)
        return Promise.resolve(juzPagesCache);
    if (juzPagesPromise)
        return juzPagesPromise;
    juzPagesPromise = fetch('/data/juz_pages.json')
        .then((r) => {
        if (!r.ok)
            throw new Error(`juz_pages: ${r.status}`);
        return r.json();
    })
        .then((data) => {
        const arr = Array.isArray(data) ? data : [];
        juzPagesCache = arr.length === 30 ? arr : [];
 ...
 */
export function getJuzForPage(pageNum, juzPages) {
    if (!juzPages.length || pageNum < 1 || pageNum > 604)
        return 1;
 ...
 */
export function getJuzForVerseKey(verseKey, mushafPages, juzPages) {
    if (!verseKey?.trim() || !mushafPages.length || !juzPages.length)
        return 1;
 ...
 */
export function getSurahNumberForPage(pageNum, mushafPages) {
    if (!mushafPages.length || pageNum < 1 || pageNum > 604)
        return 1;
    const index = pageNum - 1;
    if (index >= mushafPages.length)
        return 114;
 ...
 */
export function getPageNumbersForJuzAndSurah(juz, surah, mushafPages, juzPages) {
    ...
    if (juz === 'all' && surah !== 'all') {
        ...
        return mushafPages
            .filter((p) => compareVerseKeys(p.start_verse_key, surahLast) <= 0 &&
    ...
    if (juz !== 'all' && surah === 'all') {
        ...
        if (!entry)
            return mushafPages.map((p) => p.page);
        return Array.from({ length: entry.end_page - entry.start_page + 1 }, (_, i) => entry.start_page + i);
    }
    ...
    const lastAyah = VERSE_COUNT_PER_SURAH[surah - 1];
    const surahLast = lastAyah != null ? `${surah}:${lastAyah}` : surahFirst;
    const pagesInJuz = mushafPages.filter((p) => p.page >= entry.start_page && p.page <= entry.end_page);
    return pagesInJuz
        .filter((p) => compareVerseKeys(p.start_verse_key, surahLast) <= 0 &&
        compareVerseKeys(p.end_verse_key, surahFirst) >= 0)
        .map((p) => p.page)
 ...
 * @param lang 'en' | 'ar'
 */
export function getSurahNameForPage(surahNumber, surahData, lang) {
    if (!surahData || surahNumber < 1 || surahNumber > 114)
