...
class QuranFontLoader {
    private loadedFonts: Set<number> = new Set();
    private fontBasePath: string = '/fonts'; // Vite serves from public/ automatically
     ...
     */
    loadPageFont(pageNumber: number): void {
     ...
     */
    testFontFile(fontPath: string, fontName: string): void {
     ...
     */
    forceFontLoad(pageNumber: number): void {
        if (!document.fonts) {
     ...
     */
    preloadNearbyPages(currentPage: number): void {
     ...
     */
    clearUnusedFonts(currentPage: number, range: number = 5): void {
        let clearedCount = 0;
