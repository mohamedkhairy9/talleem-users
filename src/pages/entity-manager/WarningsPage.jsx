...
const WarningsPage = () => {
    ...
    return (<div className="flex min-h-full flex-col space-y-6">
            <PageHeader title={t('warning.listTitle', 'Warnings')} subtitle={t('warning.listDescription', 'Manage and view all warnings')} actions={[
            {
            ...
            {showCreateForm ? (<div className="bg-white rounded-lg shadow-sm p-6">
                    <div className="mb-6 flex justify-between items-center">
                        <h2 className="text-xl font-bold text-gray-900 mb-2">
                            {t('warning.createTitle', 'Create New Warning')}
                        </h2>
                        <Button onClick={() => setShowCreateForm(false)} className="text-sm text-gray-600 hover:text-gray-900" variant="outline">
                            {t('common.back', 'Back to List')}
                        </Button>
                    </div>
                    <CreateWarningForm onSuccess={() => setShowCreateForm(false)} onCancel={() => setShowCreateForm(false)}/>
                </div>) : (<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg bg-white shadow-sm p-6">
                    <WarningsList />
                </div>)}
        </div>);
};
