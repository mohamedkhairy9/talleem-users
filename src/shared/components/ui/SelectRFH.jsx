...
function SelectRFH({ label, name, control, error, options = [], isMulti = false, disabled = false, width, defaultValue, classes = '', placeholder = 'common.select', required = false, loading = false, loadOptions, defaultOptions = true, cacheOptions = true, isAsync = false }) {
    ...
    const loadAndCacheOptions = async (inputValue = '') => {
        ...
        try {
            ...
            const normalizedLoaded = Array.isArray(loaded) ? loaded : [];
            setAsyncOptions(normalizedLoaded);
    ...
    useEffect(() => {
        if (isAsync && loadOptions && fieldValue !== null && fieldValue !== undefined) {
            const loadSelectedOption = async () => {
                try {
                    ...
                    const allOptions = await loadAndCacheOptions('');
                    if (isMulti) {
        ...
        else if (isAsync && loadOptions && defaultOptions === true && asyncOptions.length === 0) {
            loadAndCacheOptions('');
        }
    ...
    return (<div>
            ...
            // Use controlled Select with async-loaded options when loadOptions is provided
            if (isAsync && loadOptions) {
                return (<Select {...commonProps} value={selectedValue} options={asyncOptions} isLoading={loading || isAsyncLoading} isSearchable={true} filterOption={() => true} noOptionsMessage={() => isAsyncLoading ? t('common.loading', 'Loading...') : t('common.noOptions', 'No options found')} onMenuOpen={() => {
                        if (defaultOptions === true && asyncOptions.length === 0) {
                            loadAndCacheOptions('');
                        }
                    }} onInputChange={(inputValue, actionMeta) => {
                        if (actionMeta.action === 'input-change') {
                            loadAndCacheOptions(inputValue);
                        }
                        return inputValue;
                    }} 
