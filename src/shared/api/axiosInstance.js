...
axiosInstance.interceptors.request.use((config) => {
    ...
    if (config.headers) {
        config.headers['X-Date-Format'] = config.headers['X-Date-Format'] ?? currentDateFormat;
        if (actingRole) {
    ...
    const isHalaqaDetailRequest = config.method?.toLowerCase() === 'get' &&
        /^\/halaqas\/[^/]+$/.test(config.url || '');
    if (isHalaqaDetailRequest) {
        console.log('Halaqa detail request debug:', {
            url: config.url,
            method: config.method,
            hasAuthorizationHeader: !!config.headers?.Authorization,
            acceptLanguage: config.headers?.['Accept-Language'],
            xDateFormat: config.headers?.['X-Date-Format'],
            xActAsRole: config.headers?.['X-Act-As-Role'] ?? null,
            xEntityId: config.headers?.['X-Entity-Id'] ?? null,
...
axiosInstance.interceptors.response.use((response) => {
    const isHalaqaDetailRequest = response.config?.method?.toLowerCase() === 'get' &&
        /^\/halaqas\/[^/]+$/.test(response.config?.url || '');
    if (isHalaqaDetailRequest) {
        console.log('Halaqa detail response debug:', {
    ...
    const isHalaqaDetailRequest = error.config?.method?.toLowerCase() === 'get' &&
        /^\/halaqas\/[^/]+$/.test(error.config?.url || '');
    ...
    if (isHalaqaDetailRequest) {
        console.log('Halaqa detail error debug:', {
