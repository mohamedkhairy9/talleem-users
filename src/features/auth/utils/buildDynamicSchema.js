...
function normalizeVisibleWhenValue(v) {
 ...
 */
export const buildDynamicSchema = (fields) => {
    const schemaObject = {};
    const buildFieldSchema = (field) => {
        ...
        if (field.key === 'entity_id' && field.required && field.type === 'select') {
            fieldSchema = fieldSchema.when(['branch_id', 'main_program_id'], {
                is: (branch_id, main_program_id) => branch_id != null && branch_id !== '' &&
        ...
        if (field.visible_when && Object.keys(field.visible_when).length > 0) {
            ...
            const allowedByKey = field.visible_when;
