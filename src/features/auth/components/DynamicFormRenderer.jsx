...
function itemToOption(item, currentLang) {
...
function collectDependsOnFields(fields, prefix = '') {
    const result = [];
...
function collectVisibleWhenFields(fields, prefix = '') {
    const result = [];
    for (const field of fields) {
        const fieldPath = prefix ? `${prefix}.${field.key}` : field.key;
        if (field.visible_when && Object.keys(field.visible_when).length > 0) {
            const visibleWhen = Object.fromEntries(Object.entries(field.visible_when).map(([k, v]) => [k, (v || []).map(String)]));
 ...
 */
const DynamicFormRenderer = ({ fields, control, errors, setValue, joinRequestRole }) => {
    ...
    useEffect(() => {
        if (!setValueFn || typeof setValueFn !== 'function')
            return;
    ...
    const [cachedEntityOption, setCachedEntityOption] = useState(null);
    useEffect(() => {
        if (formValues?.entity_id == null || formValues?.entity_id === '')
            setCachedEntityOption(null);
        else if (cachedEntityOption && cachedEntityOption.value !== formValues?.entity_id)
            setCachedEntityOption(null);
    }, [formValues?.entity_id, cachedEntityOption?.value]);
    const { i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
    // Paginated loadOptions (same approach as booking-pro-dashboard-dev)
    const loadBranches = useCallback(async (search, _loaded, additional) => {
    ...
    const loadMainPrograms = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadCities = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadSessionModes = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadNationalities = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadMajors = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadAcademicQualifications = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadMemorizationProgramEntityTypes = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadNeighborhoods = useCallback(async (search, _loaded, additional) => {
        if (!cityId)
    ...
    const loadEntities = useCallback(async (search, _loaded, additional) => {
    ...
    const loadEducationProgramEntityTypes = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const loadActivities = useCallback(async (search, _loaded, additional) => {
        if (!mainProgramId)
            return { options: [], hasMore: false, additional: { page: 1 } };
        const page = additional?.page ?? 1;
    ...
    const loadLocationTypes = useCallback(async (search, _loaded, additional) => {
        const page = additional?.page ?? 1;
    ...
    const isFieldVisible = (field, prefix) => {
        if (!field.visible_when)
            return true;
        const allowedValuesNormalized = (val) => (val || []).map((v) => String(v));
        for (const [depField, allowedValues] of Object.entries(field.visible_when)) {
            ...
            if (currentValue === undefined || currentValue === null || currentValue === '')
                return false;
            if (!allowed.includes(String(currentValue)))
                return false;
        }
        return true;
    };
    ...
    const isFieldDisabled = (field, prefix) => {
        if (field.disabled)
            return true;
    ...
    const renderField = (field, prefix = '') => {
        ...
        switch (field.type) {
            ...
            case 'date':
                ...
                return (<FormInput key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} type={field.type} required={field.required} error={fieldError?.message} disabled={isDisabled}/>);
            case 'textarea':
                return (<FormTextarea key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} required={field.required} error={fieldError?.message} rows={3} disabled={isDisabled}/>);
            case 'boolean':
                return (<FormCheckbox key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} required={field.required} error={fieldError?.message} disabled={isDisabled}/>);
            case 'file': {
                const filesHint = joinRequestRole &&
                    field.key === 'files' &&
                    requiredDocumentsData?.documents &&
                    requiredDocumentsData.documents.length > 0
                    ? requiredDocumentsData.documents.join('، ')
                    : undefined;
                return (<FormFile key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} required={field.required} error={fieldError?.message} multiple={field.multiple} accept={field.accept} disabled={isDisabled} hint={filesHint}/>);
            }
            case 'object': {
                // Multilingual object (name.ar, name.en)
                const objectLabel = extractLabel(field.label);
                return (<div key={field.key} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormInput name={`${fieldName}.ar`} control={control} label={`${objectLabel} (Arabic)`} required={field.required} error={fieldError?.ar?.message} disabled={isDisabled}/>
                        <FormInput name={`${fieldName}.en`} control={control} label={`${objectLabel} (English)`} required={field.required} error={fieldError?.en?.message} disabled={isDisabled}/>
                    </div>);
            }
            case 'select':
            case 'multiselect': {
                ...
                if (field.options) {
                    ...
                    return (<FormSelect key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} required={field.required} options={staticOptions} error={fieldError?.message} isMulti={field.type === 'multiselect'} isDisabled={isDisabled}/>);
                }
                // Handle dynamic select with paginated API (same approach as booking-pro-dashboard-dev)
                const loadOptionsMap = {
                ...
                if (field.key === 'entity_id') {
                    return (<Controller key={field.key} name={fieldName} control={control} render={({ field: f }) => {
                            const entityDisplayValue = f.value != null && f.value !== ''
                                ? (cachedEntityOption && cachedEntityOption.value === f.value
                                    ? cachedEntityOption
                                    : { value: f.value, label: String(f.value) })
                                : null;
                            return (<div>
                                        {extractLabel(field.label) && (<label className="block text-sm font-medium text-gray-700 mb-1">
                                                {extractLabel(field.label)}
                                                {field.required && <span className="text-red-500 ml-1">*</span>}
                                            </label>)}
                                        <AsyncPaginate value={entityDisplayValue} loadOptions={loadOptions} onChange={(option) => {
                                    const single = option;
                                ...
                                }} additional={{ page: 1 }} defaultAdditional={{ page: 1 }} getOptionLabel={(o) => o.label} getOptionValue={(o) => String(o.value)} isClearable isDisabled={isDisabled} placeholder={t('common.select', 'Select an option')} classNamePrefix="react-select" debounceTimeout={300} styles={{
                                    control: (base, state) => ({
                                        ...base,
                                    ...
                                    menu: (base) => ({ ...base, zIndex: 9999 })
                                        ...
                                        {entityIdTouched && (fieldError?.message) && (<p className="mt-1 h-4 text-xs text-red-600">{fieldError.message}</p>)}
                                    </div>);
                        }}/>);
                }
                return (<FormAsyncPaginate key={field.key} name={fieldName} control={control} label={extractLabel(field.label)} required={field.required} loadOptions={loadOptions} error={fieldError?.message} isDisabled={isDisabled} isMulti={field.type === 'multiselect'} placeholder={t('common.select', 'Select an option')} defaultAdditional={{ page: 1 }}/>);
            }
            case 'group': {
                ...
                return (<div key={field.key} className="border rounded-lg p-4 space-y-3 bg-gray-50">
    ...
    const orderedFields = React.useMemo(() => {
        ...
        const rest = [];
        fields.forEach((f) => {
            if (ENTITY_KEYS.includes(f.key))
                entityFields.push(f);
            else if (LOCATION_KEYS.includes(f.key))
                locationFields.push(f);
            else if (f.key === 'manager')
                managerField.push(f);
            else
                rest.push(f);
        });
    ...
    const renderFieldWrapper = (field) => {
        const content = renderField(field);
        if (content == null)
            return null;
        const isFullWidth = ['textarea', 'file', 'object', 'group'].includes(field.type);
        return (<div key={field.key} className={isFullWidth ? 'col-span-full' : ''}>
                {content}
                {(field.note || field.notes) && (<p className="mt-1 text-xs text-gray-500 italic">{extractLabel(field.note || field.notes)}</p>)}
            </div>);
    };
    ...
    const branchMainProgramFields = BRANCH_MAIN_PROGRAM_KEYS.map((key) => fields.find((f) => f.key === key)).filter(Boolean);
    const entityIdFields = orderedFields.filter((f) => f.key === 'entity_id');
    const restFields = orderedFields.filter((f) => !ENTITY_KEYS.includes(f.key) &&
    ...
    const mapPickerBlock = hasMapFields ? (<div key="map-picker" className="col-span-full mt-6 space-y-3">
        ...
        }} oldLocation={latitude && longitude
            ? { lat: parseFloat(latitude.toString()), lng: parseFloat(longitude.toString()) }
            : null} disabled={false}/>
            {(errors.latitude || errors.longitude) && (<div className="mt-1">
