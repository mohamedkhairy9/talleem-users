 ...
 */
export const useNeighborhoodsOptions = (cityId) => {
    const { i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
 ...
 */
export const useEntitiesOptions = (branchId, mainProgramId) => {
    const { i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
    const query = useQuery({
        queryKey: ['formOptions', 'entities', branchId, mainProgramId],
        queryFn: () => registrationService.getEntities(branchId && mainProgramId
