    ...
    return (<div className="space-y-4 overflow-y-auto p-2">
            {list.map((row) => (<div key={row.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                    <div className="mb-2 flex items-start justify-between gap-2">
                        <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${row.status === 'approved'
                ...
                : row.status === 'rejected' || row.status === 'cancelled'
                    ...
                    <dl className="space-y-1.5 text-sm">
                        ...
                        {row.medical_report_url && <div className="flex justify-between gap-2">
                            ...
                            <span className="text-end text-gray-900">
                                <ImageWithViewer src={row.medical_report_url} alt={t('leaves.medicalReport', 'Medical Report')} imgClassName="w-10 h-10 object-cover rounded-full" fallback="-"/>
                            </span>
                    ...
                    {row.status === 'pending' && (<div className="mt-3">
                            <button type="button" onClick={() => onCancel(row)} disabled={cancellingId === row.id} className="text-sm font-medium text-red-600 hover:text-red-700 disabled:opacity-50">
