...
import type { TeacherLeaveItem, CreateLeavePayload } from '../types/teacher-leaves.types';
...
export function useTeacherLeaves() {
    ...
    const list: TeacherLeaveItem[] = Array.isArray(responseBody?.data) ? responseBody.data : [];
        ...
        mutationFn: (payload: CreateLeavePayload) => {
            ...
            if (payload.leave_type === 'leave') {
                ...
                if (files && (Array.isArray(files) ? files.length > 0 : files.length > 0)) {
                    ...
                    if (file instanceof File) formData.append('medical_report', file);
                }
            } else {
...
export function useCancelTeacherLeave() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id: number) => teacherLeavesService.cancelLeave(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['teacher-leaves'] });
        }
    });
