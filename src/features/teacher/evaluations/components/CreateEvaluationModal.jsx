...
function buildDefaultValues(criteria, canAttachFiles) {
    const out = {
        evaluated_id: null,
        evaluation_date: '',
        attachment: canAttachFiles ? null : undefined
    };
    criteria.forEach((c) => {
        out[`score_${c.id}`] = '';
        out[`notes_${c.id}`] = '';
...
function buildFormData(templateId, evaluatedEntity, criteria, values, canAttachFiles) {
...
const CreateEvaluationModal = ({ isOpen, onClose, onSuccess }) => {
    const { t, i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
    const [selectedTemplateId, setSelectedTemplateId] = useState(0);
    const { templates, isLoading: isLoadingTemplates } = useEvaluationTemplates(isOpen);
    const { detail, isLoading: isLoadingDetail } = useEvaluationTemplate(selectedTemplateId > 0 ? selectedTemplateId : null);
    ...
    const criteria = useMemo(() => (template?.criteria ? [...template.criteria] : []), [template?.criteria]);
    const canAttachFiles = detail?.can_attach_files ?? false;
    const availableEntities = detail?.available_entities ?? [];
    const defaultValues = useMemo(() => buildDefaultValues(criteria, canAttachFiles), [criteria, canAttachFiles]);
    ...
    const getEvaluatedEntity = useCallback(() => {
    ...
    const onSubmit = useCallback((values) => {
        ...
        const formData = buildFormData(selectedTemplateId, entity, criteria, values, canAttachFiles);
    ...
    return (<div className="fixed inset-0 z-[60] overflow-y-auto">
            ...
            <div className="relative flex min-h-full items-center justify-center p-4 pt-20 md:pt-24">
                <div className="relative w-full max-w-lg rounded-lg bg-white shadow-xl z-10 max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    {showTemplateStep && (<div className="px-6 py-4 space-y-4 flex-shrink-0">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    {t('evaluations.template', 'Evaluation template')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <ReactSelect value={selectedTemplateId || null} onChange={(v) => setSelectedTemplateId(v != null ? Number(v) : 0)} options={templateOptions} placeholder={t('common.select', 'Select an option')} isDisabled={isLoadingTemplates} menuPortalTarget={typeof document !== 'undefined' ? document.body : null} menuPosition="fixed"/>
                            </div>
                    ...
                    {selectedTemplateId > 0 && isLoadingDetail && (<div className="px-6 py-8 text-center text-gray-500 flex-1">
                            {t('common.loading', 'Loading...')}
                        </div>)}

                    {showForm && template && (<form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4 space-y-4 flex-1 overflow-y-auto">
                            <div className="flex items-center justify-between gap-2">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => setSelectedTemplateId(0)}>
                                    {t('evaluations.changeTemplate', 'Change template')}
                                </Button>
                            </div>
                            {entityOptions.length === 0 ? (<p className="text-sm text-amber-700 bg-amber-50 rounded-lg p-3">
                                    {t('evaluations.noEntitiesToEvaluate', 'No one available to evaluate for this template.')}
                                </p>) : (<FormSelect name="evaluated_id" control={control} label={t('evaluations.evaluateWho', 'Evaluate')} required options={entityOptions} placeholder={t('common.select', 'Select an option')} error={errors.evaluated_id?.message} menuPortalTarget={typeof document !== 'undefined' ? document.body : null} menuPosition="fixed"/>)}

                            <FormInput name="evaluation_date" control={control} type="date" label={t('evaluations.evaluationDate', 'Evaluation date')}/>
                            ...
                            <div className="border-t border-gray-200 pt-3">
                                ...
                                <div className="space-y-3">
                                    {criteria.map((c) => (<div key={c.id} className="rounded-lg border border-gray-200 bg-gray-50/50 p-3 space-y-2">
                                            <p className="text-sm font-medium text-gray-900">
                                                {getLocalizedText(c.criteria_name, currentLang, '-')} (
                                                {t('evaluations.maxDegree', 'max')} {c.degree})
                                            </p>
                                            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                                <FormInput name={`score_${c.id}`} control={control} type="number" label={t('evaluations.score', 'Score')} required error={errors[`score_${c.id}`]?.message}/>
                                                <FormTextarea name={`notes_${c.id}`} control={control} label={t('evaluations.notes', 'Notes')} placeholder={t('evaluations.notesPlaceholder', 'Optional')} rows={1}/>
                                            </div>
                                        </div>))}
                                </div>
                            </div>

                            {canAttachFiles && (<FormFile name="attachment" control={control} label={t('evaluations.attachments', 'Attachments')} accept="image/*,.pdf"/>)}
                            ...
                            <div className="flex justify-end gap-3 border-t border-gray-200 bg-gray-50 -mx-6 px-6 py-4 mt-4 flex-shrink-0">
                                ...
                                <Button type="submit" variant="primary" loading={submitMutation.isPending} disabled={entityOptions.length === 0}>
