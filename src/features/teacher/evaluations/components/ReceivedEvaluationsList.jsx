...
const ReceivedEvaluationsList = () => {
    ...
    const columns = useMemo(() => createReceivedEvaluationsListColumns({ t, getLocalizedText }), [t, getLocalizedText]);
    if (error) {
    ...
    return (<div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <div className="flex flex-1 flex-col overflow-hidden md:hidden min-h-[280px] bg-white rounded-lg">
                <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden">
                    <ReceivedEvaluationsListMobile list={list} isLoading={isLoading} hasError={!!error} errorMessage={error ? t('evaluations.loadError', 'Error loading evaluations.') : undefined} emptyMessage={t('evaluations.noEvaluations', 'No evaluations found.')} getLocalizedText={getLocalizedText}/>
                </div>
            </div>

            <div className="hidden md:flex flex-col flex-1 min-h-0 overflow-hidden">
                <div className="flex-1 min-h-0 overflow-hidden">
                    <Table columns={columns} data={list} loading={isLoading} emptyMessage={t('evaluations.noEvaluations', 'No evaluations found.')} scrollable/>
                </div>
            </div>
        </div>);
};
