...
import { AlertTriangleIcon, CalendarIcon, CircleIcon, XIcon } from '@/shared/icons';
...
const TeacherHalaqaDetailPage = () => {
    ...
    const handleSubmitTeacherAbsence = () => {
        if (!selectedReason || !students || students.length === 0)
            return;
    ...
    const errorMessage = error
        ? error?.message || error?.data?.message || t('halaqa.loadErrorDetail')
        : null;
    // Check if error is related to time restrictions
    const isTimeRestrictionError = errorMessage && (errorMessage.toLowerCase().includes('cannot record') ||
        errorMessage.toLowerCase().includes('time window') ||
        errorMessage.toLowerCase().includes('allowed time') ||
        error?.data?.can_record === false);
    ...
    return (<div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {students && students.length > 0 && (<div className="space-y-4">
                            {/* Teacher Absence Button */}
                            <div className="flex justify-end">
                                <Button type="button" variant="outline" onClick={handleOpenTeacherAbsenceModal} className="flex items-center gap-2">
                                    <AlertTriangleIcon width={16} height={16}/>
                                    {t('attendance.teacherAbsence')}
                                </Button>
                            </div>

                            <TeacherHalaqaStudents students={students} isLoading={isLoading} error={error} getLocalizedText={getLocalizedText} halaqaId={id} attendanceTypes={attendanceTypes} currentDate={date}/>
                        </div>)}
                </div>
                ...
                <div className="space-y-6">
                    ...
                    {/* Session Info */}
                    {date && (<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            {showTeacherAbsenceModal && (<div className="fixed inset-0 z-[60] overflow-y-auto">
                    {/* Backdrop */}
                    <div className="fixed inset-0 bg-black transition-opacity" style={{ opacity: 0.5 }} onClick={handleCloseTeacherAbsenceModal} aria-hidden="true"/>
                    ...
                    <div className="relative flex min-h-full items-center justify-center p-4 pt-20 md:pt-24">
                        <div className="relative w-full max-w-md transform overflow-hidden rounded-lg bg-white shadow-xl transition-all z-10">
                            ...
                            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                                ...
                                <button type="button" onClick={handleCloseTeacherAbsenceModal} className="rounded-lg p-1 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600" aria-label={t('common.closeAria')}>
                            ...
                            <div className="px-6 py-4">
                                ...
                                <div className="space-y-3">
                                    <label className="block">
                                        <input type="radio" name="absence-reason" value="teacher_absence" checked={selectedReason === 'teacher_absence'} onChange={(e) => setSelectedReason(e.target.value)} className="sr-only"/>
                                        <div className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${selectedReason === 'teacher_absence'
                ? 'border-primary-500 bg-primary-50'
                : 'border-gray-200 hover:border-gray-300'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedReason === 'teacher_absence'
                ? 'border-primary-500 bg-primary-500'
                : 'border-gray-300'}`}>
                                                    {selectedReason === 'teacher_absence' && (<div className="w-2 h-2 rounded-full bg-white"></div>)}
                                                </div>
                                                <div>
                                    ...
                                    <label className="block">
                                        <input type="radio" name="absence-reason" value="force_majeure" checked={selectedReason === 'force_majeure'} onChange={(e) => setSelectedReason(e.target.value)} className="sr-only"/>
                                        <div className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${selectedReason === 'force_majeure'
                ? 'border-primary-500 bg-primary-50'
                : 'border-gray-200 hover:border-gray-300'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedReason === 'force_majeure'
                ? 'border-primary-500 bg-primary-500'
                : 'border-gray-300'}`}>
                                                    {selectedReason === 'force_majeure' && (<div className="w-2 h-2 rounded-full bg-white"></div>)}
                                                </div>
                                                <div>
                                                    <p className="font-semibold text-gray-900">
                                ...
                                {students && students.length > 0 && (<div className="mt-4 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex items-center justify-end gap-3 border-t border-gray-200 bg-gray-50 px-6 py-4">
                                <Button variant="outline" onClick={handleCloseTeacherAbsenceModal} disabled={bulkAttendanceMutation.isPending}>
                                    {t('common.cancel')}
                                </Button>
                                <Button variant="primary" onClick={handleSubmitTeacherAbsence} loading={bulkAttendanceMutation.isPending} disabled={!selectedReason}>
                                    {t('attendance.submit')}
                                </Button>
                            </div>
                        </div>
