...
const TeacherRequestsList = ({ onView }) => {
    ...
    const columns = useMemo(() => createTeacherRequestsColumns({ t, currentLang, onView }), [t, currentLang, onView]);
    if (error) {
    ...
    return (<div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <div className="flex-1 min-h-0 overflow-hidden">
                <Table data={list} columns={columns} loading={isLoading} emptyMessage={t('teacherRequests.noRequests', 'No requests yet.')} scrollable/>
            </div>

            {totalPages > 1 && (<div className="flex-shrink-0 border-t border-gray-200 pt-3 pb-4">
