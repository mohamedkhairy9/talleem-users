...
function DynamicFormField({ field, control, setValue, error }) {
    ...
    const dependsOnFieldName = selectSource?.source === 'entities' ? selectSource.dependsOnField : 'request_type_id';
    const dependsOnValue = useWatch({ control, name: dependsOnFieldName });
    const branchIdForEntities = selectSource?.source === 'entities' ? (dependsOnValue != null && dependsOnValue !== '' ? Number(dependsOnValue) : null) : null;
    const branchesLoadOptions = useMemo(() => createBranchesLoader(currentLang), [currentLang]);
    const entitiesLoadOptions = useMemo(() => createEntitiesLoader(branchIdForEntities, currentLang), [branchIdForEntities, currentLang]);
    useEffect(() => {
    ...
    if (field.type === 'select' && selectSource) {
        ...
        if (!loadOptions)
            return null;
        return (<FormAsyncPaginate name={field.key} control={control} label={field.label} required={field.required} placeholder={t('common.select', 'Select an option')} error={error} isDisabled={isDisabled} loadOptions={loadOptions} defaultAdditional={{ page: 1 }}/>);
    }
    const type = field.type === 'date' ? 'date' : field.type === 'number' ? 'number' : 'text';
    return (<FormInput name={field.key} control={control} type={type} label={field.label} required={field.required} error={error}/>);
}
/** Full form (request type + dynamic fields); owns form state and only mounts when formDef is loaded */
function CreateRequestFormBody({ formDef, requestTypeId, requestTypeOptions, isLoadingTypes, onRequestTypeChange, onClose, onSuccess }) {
    const { t } = useTranslation();
    const createMutation = useCreateTeacherRequest();
    const schema = useMemo(() => buildCreateRequestFormSchema(formDef.data.fields), [formDef.data.fields]);
    const defaultValues = useMemo(() => buildCreateRequestDefaultValues(requestTypeId, formDef.data.fields), [requestTypeId, formDef.data.fields]);
    ...
    const getValidationError = useCallback((message) => message ? t(`teacherRequests.validation.${message}`, message) : undefined, [t]);
    const fields = useMemo(() => [...(formDef.data.fields || [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0)), [formDef.data.fields]);
    const onSubmit = useCallback((data) => {
        const request_type_id = Number(data.request_type_id);
        const submittedData = {};
        Object.keys(data).forEach((k) => {
            if (k === 'request_type_id')
                return;
            const v = data[k];
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4 space-y-4">
            <FormSelect name="request_type_id" control={control} label={t('teacherRequests.requestType', 'Request Type')} required options={requestTypeOptions} placeholder={t('common.select', 'Select an option')} error={getValidationError(errors.request_type_id?.message)} isDisabled={isLoadingTypes}/>

            {fields.map((field) => (<DynamicFormField key={field.key} field={field} control={control} setValue={setValue} error={getValidationError(errors[field.key]?.message)}/>))}
...
const CreateRequestModal = ({ isOpen, onClose, onSuccess }) => {
    const { t, i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
    const [selectedRequestTypeId, setSelectedRequestTypeId] = useState(0);
    const { data: requestTypesData, isLoading: isLoadingTypes } = useRequestTypes();
    const { data: formDef, isLoading: isLoadingForm } = useJoinRequestForm(selectedRequestTypeId > 0 ? selectedRequestTypeId : null);
    ...
    const onRequestTypeChange = useCallback((id) => {
            ...
            <div className="relative flex min-h-full items-center justify-center p-4 pt-20 md:pt-24">
                <div className="relative w-full max-w-md rounded-lg bg-white shadow-xl z-10">
                    <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                        ...
                        <button type="button" onClick={handleClose} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600" aria-label={t('common.closeAria')}>
                            <XIcon width={20} height={20}/>
                        </button>
                    </div>

                    {showOnlyRequestType && (<div className="px-6 py-4 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    {t('teacherRequests.requestType', 'Request Type')}{' '}
                                    <span className="text-red-500">*</span>
                                </label>
                                <ReactSelect value={selectedRequestTypeId ? selectedRequestTypeId : null} onChange={(v) => setSelectedRequestTypeId(v != null ? Number(v) : 0)} options={requestTypeOptions} placeholder={t('common.select', 'Select an option')} isDisabled={isLoadingTypes}/>
                            </div>
                            <div className="flex justify-end gap-3 border-t border-gray-200 bg-gray-50 -mx-6 px-6 py-4 mt-4">
                    ...
                    {selectedRequestTypeId > 0 && isLoadingForm && (<div className="px-6 py-8 text-center text-gray-500">
                            {t('common.loading', 'Loading...')}
                        </div>)}

                    {showFullForm && formDef && (<CreateRequestFormBody key={formDef.id} formDef={formDef} requestTypeId={selectedRequestTypeId} requestTypeOptions={requestTypeOptions} isLoadingTypes={isLoadingTypes} onRequestTypeChange={onRequestTypeChange} onClose={handleClose} onSuccess={onSuccess}/>)}
                </div>
            </div>
        </div>);
};
