...
import type { SelectOption } from '@/globals/types';

const PER_PAGE = 20;

/** Return type for FormAsyncPaginate loadOptions */
export type AsyncPaginateLoadResult = {
...
function transformToSelectOptions(data: any[], currentLang: string): SelectOption[] {
    if (!data) return [];
 ...
 */
export function useJoinRequestForm(id: number | null) {
    return useQuery({
 ...
 * Only runs when id is truthy.
 */
export function useTeacherRequestDetail(id: number | null) {
    return useQuery({
...
export function useBranchesOptions() {
    ...
    const options = useMemo(
        () => transformToSelectOptions(extractDataList(query.data), lang),
        [query.data, lang]
    );
 ...
 */
export function useEntitiesOptions(branchId: number | null) {
    const { i18n } = useTranslation();
    const lang = i18n.language || 'ar';
    const query = useQuery({
 ...
 */
export function createBranchesLoader(
    currentLang: string
): (search: string, _loaded: unknown, additional?: { page?: number }) => Promise<AsyncPaginateLoadResult> {
 ...
 */
export function createEntitiesLoader(
...
export function getSelectOptionsSource(
    field: JoinRequestFormField
): { source: 'branches' } | { source: 'entities'; dependsOnField: string } | null {
    if (field.type !== 'select') return null;
    const key = field.key.toLowerCase();
    if (key.includes('branch') || key === 'new_branch_id') return { source: 'branches' };
