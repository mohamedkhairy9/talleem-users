 ...
 */
const TeacherCertificatesList = () => {
    const { t, i18n } = useTranslation();
    const currentLang = i18n.language || 'ar';
    const [page, setPage] = useState(1);
    const [certificatesStudentId, setCertificatesStudentId] = useState(null);
    ...
    const onViewCertificates = useCallback((row) => {
        setCertificatesStudentId(row.id);
    }, []);
    const columns = useMemo(() => createTeacherCertificatesListColumns({ t, getLocalizedText, onViewCertificates }), [t, getLocalizedText, onViewCertificates]);
    ...
    return (<>
            <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
                ...
                <div className="flex flex-1 flex-col overflow-hidden md:hidden min-h-[280px] bg-white rounded-lg">
                    <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden">
                        <TeacherCertificatesListMobile list={list} isLoading={isLoading} hasError={!!error} errorMessage={error ? t('certificates.loadError', 'Error loading students.') : undefined} emptyMessage={t('certificates.noStudents', 'No students yet.')} getLocalizedText={getLocalizedText} onViewCertificates={onViewCertificates}/>
                    </div>
                    {totalPages > 1 && (<div className="flex-shrink-0 border-t border-gray-200 pt-3 px-4 pb-4">
                            <Pagination currentPage={currentPage} totalPages={totalPages} perPage={meta?.per_page ?? PER_PAGE} total={total} onPageChange={setPage}/>
                        </div>)}
                </div>
                ...
                <div className="hidden md:flex flex-col flex-1 min-h-0 overflow-hidden">
                    <div className="flex-1 min-h-0 overflow-hidden">
                        <Table columns={columns} data={list} loading={isLoading} emptyMessage={t('certificates.noStudents', 'No students yet.')} scrollable/>
                    </div>
            ...
            <StudentCertificatesModal isOpen={certificatesStudentId != null} studentId={certificatesStudentId} onClose={() => setCertificatesStudentId(null)}/>
        </>);
};
