...
interface CalendarHalaqasListProps {
    getLocalizedText: (obj: { ar?: string; en?: string } | string | null | undefined) => string;
}

const CalendarHalaqasList: React.FC<CalendarHalaqasListProps> = ({ getLocalizedText }) => {
    ...
    if (error) {
        return (
            <div className="rounded-lg bg-white p-8 text-center text-red-600 shadow-sm border border-gray-200">
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {halaqas.map((item: CalendarHalaqaItem) => (
                <div
                    key={item.id}
