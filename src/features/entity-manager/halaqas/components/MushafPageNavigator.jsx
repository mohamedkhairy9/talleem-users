 ...
 */
const MushafPageNavigator = ({ value, onChange, pageNumbers, className = '', disabled = false }) => {
    ...
    const navBtn = 'flex items-center justify-center w-10 h-10 shrink-0 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors';
    return (<div className={`flex items-center gap-2 min-w-0 max-w-full ${isRtl ? 'flex-row-reverse' : ''} ${className}`}>
            <button type="button" onClick={goNext} disabled={disabled || !canGoNext} className={navBtn} aria-label={t('quran.nextPage', 'Next page')}>
                <ChevronRightIcon width={20} height={20} className={isRtl ? 'rotate-180' : ''}/>
            </button>
            <div className="min-w-0 flex-1">
                <ReactSelect value={value} onChange={(val) => onChange(val != null ? Number(val) : value)} options={options} placeholder={t('quran.selectPage', 'Select page')} isDisabled={disabled}/>
            </div>
            <button type="button" onClick={goPrev} disabled={disabled || !canGoPrev} className={navBtn} aria-label={t('quran.previousPage', 'Previous page')}>
                <ChevronRightIcon width={20} height={20} className={isRtl ? '' : 'rotate-180'}/>
            </button>
        </div>);
};
