...
const InlineMushafSegmentPicker = ({ selectedStartSegment, selectedEndSegment, onSelectStartSegment, onSelectEndSegment, planType, hideInlineMushaf = false, onPageChange, selectionVerseKeyFromOutside, onCurrentSelectionChange, onNavigablePagesChange, viewerPageSync }) => {
    ...
    useEffect(() => {
        if (viewerPageSync == null || viewerPageSync < 1)
            return;
        ...
        const loadPage = (pageNum) => {
            ...
            try {
                ...
                const lines = firstResult.values.map((row) => {
                    const line = {};
                    columns.forEach((col, idx) => { line[col] = row[idx]; });
                    return line;
                });
                setPageLines(lines);
            }
    ...
    if (isLoading || error) {
        return (<div className="rounded-xl border border-gray-200 bg-gray-50 p-8 text-center">
    ...
    return (<div className="space-y-4">
            ...
            : `— ${t('quran.notSet', 'Not set')}`}
                    ...
                    {planType === 'start_end' && (<div className="rounded-lg border-2 border-blue-200 bg-blue-50/50 px-3 py-2">
                ...
                <div className="border-t border-gray-100 pt-3 space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <ReactSelect value={selectedJuz} onChange={(val) => setSelectedJuz(val === null || val === 'all' ? 'all' : Number(val))} options={juzOptionsForSelect} placeholder={t('quran.juz', 'Juz')}/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">{t('quran.surah', 'Surah')}</label>
                            <ReactSelect value={selectedSurah} onChange={(val) => setSelectedSurah(val === null || val === 'all' ? 'all' : Number(val))} options={surahOptionsForSelect} placeholder={t('quran.surah', 'Surah')}/>
                        </div>
                    </div>
                    <div className="flex justify-end w-full">
                        <MushafPageNavigator value={currentPage} onChange={setCurrentPage} pageNumbers={pageNumbers.length > 0 ? pageNumbers : undefined} className="w-full"/>
                    </div>
                </div>
            </div>
            ...
            return (<div className={`grid ${gridCols} gap-4 items-start`}>
                        {!hideInlineMushaf && (
                /* Mushaf page */
                <div className="rounded-xl border border-gray-200 overflow-hidden bg-white min-w-0">
                                {isLoadingSegments && currentPageSegments.length === 0 ? (<div className="flex justify-center py-12">
                                        <div className="spinner"/>
                                    </div>) : (<MushafPage pageLines={pageLines} currentPage={currentPage} wordsDb={wordsDb} surahData={surahData} selectedAyahs={selectedAyahs} onWordClick={handleWordClick} isFontLoading={isFontLoading}/>)}
                            </div>)}
                        ...
                        return (<button key={seg.id} type="button" onClick={() => setCurrentSelection(seg)} className={`rounded-lg px-3 py-2 text-left text-sm border-2 transition-colors ${isSelected
                                ...
                                : currentSelection?.id === seg.id
                                    ...
                                    : 'border-gray-200 bg-white hover:border-primary-300'}`}>
                                                ...
                                                {(isStart || isEnd) && (<span className="ml-1 text-primary-600 text-xs block mt-0.5">
                                                        {isStart ? ` [${t('quran.startSegment', 'Start')}]` : ` [${t('quran.endSegment', 'End')}]`}
                                                    </span>)}
                                            </button>);
                    })}
                                </div>) : (<p className="text-sm text-gray-500 py-2">{t('quran.noSegmentsFound', 'No segments found for this page')}</p>)}
                        </div>
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white p-3 min-w-0 space-y-3">
                            ...
                            {currentSelection ? (<>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <button type="button" onClick={handleSetStart} className={`inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium shadow-sm transition-colors w-full ${isCurrentStart
