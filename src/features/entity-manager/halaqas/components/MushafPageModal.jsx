 ...
 */
const MushafPageModal = ({ isOpen, onClose, pageNumber, selectedAyahs = new Set(), startVerseKey, endVerseKey, onSelectVerseKey, onVerseKeyClick, embedded = false, navigablePageNumbers, onPageChange }) => {
    ...
    const [planSegmentVerseKeys, setPlanSegmentVerseKeys] = useState(new Set());
    ...
    const [selectedVerseKeyInModal, setSelectedVerseKeyInModal] = useState(null);
    ...
    const singleViewPagesList = useMemo(() => {
        if (isPlanView)
            return [];
        const list = navigablePageNumbers != null && navigablePageNumbers.length > 0
    ...
    const canSingleGoNext = !isPlanView && singleViewPageIndex >= 0 && singleViewPageIndex < singleViewPagesList.length - 1;
    const goSinglePrev = () => {
        if (!canSingleGoPrev)
            return;
    ...
    useEffect(() => {
        if (isPlanView || !singleViewPagesList.length)
            return;
    ...
    const buildPageRange = React.useCallback((startPage, endPage) => {
    ...
    const getAllPagesInRange = React.useCallback(async (startKey, endKey) => {
    ...
    const buildVerseKeysFromSegments = React.useCallback((segs) => {
        ...
        for (const seg of segs) {
            ...
            if (compareVerseKeys(segLast, startVerseKey) < 0 || compareVerseKeys(segFirst, endVerseKey) > 0)
                continue;
    ...
    useEffect(() => {
        if (!isOpen || !isPlanView || !startVerseKey || !endVerseKey)
            return;
    ...
    useEffect(() => {
        ...
        quranSegmentsService
            ...
            .catch((err) => {
            if (!cancelled)
                console.error('Error loading segments for page:', currentPage, err);
        });
    ...
    useEffect(() => {
        if (!isOpen || !linesDb || currentPage < 1 || currentPage > 604)
            return;
                ...
                if (!columns || !Array.isArray(columns)) {
                    try {
                        ...
                        if (schemaResult && schemaResult.length > 0 && schemaResult[0].columns) {
                            ...
                            columns = schemaResult[0].values.map((row) => row[1]); // column name is at index 1
                        }
                        else {
    ...
    const handleWordClick = React.useCallback((_wordId, location) => {
        if (!onSelectVerseKey || !startVerseKey || !endVerseKey)
            return;
        ...
        if (!/^\d+:\d+$/.test(verseKey))
            return;
        if (compareVerseKeys(verseKey, startVerseKey) < 0 || compareVerseKeys(verseKey, endVerseKey) > 0)
            return;
    ...
    const handleVerseKeyClick = React.useCallback((_wordId, location) => {
        if (!onVerseKeyClick)
            return;
        const parts = location.trim().split(':').filter(Boolean);
        if (parts.length < 2)
    ...
    useEffect(() => {
        if (isOpen && onSelectVerseKey)
            setSelectedVerseKeyInModal(null);
    }, [isOpen, onSelectVerseKey]);
    if (!isOpen)
        return null;
    const content = (<div className={`relative bg-white ${embedded ? 'rounded-lg border border-gray-200 w-full min-h-0' : 'overflow-hidden rounded-xl shadow-xl max-w-3xl w-full max-h-[calc(100vh-5rem)] md:max-h-[calc(100vh-6rem)]'}`}>
            {/* Header: only when not embedded (title + close); embedded has no header */}
            {!embedded && (<div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 bg-white flex-wrap gap-3">
            ...
            {onSelectVerseKey && !isLoading && !isLoadingPlanPages && !error && (<div className="px-4 sm:px-6 py-3 bg-primary-50 border-b border-primary-100">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <p className="text-sm text-primary-800">
                            {selectedVerseKeyInModal ? (<span>
                                ...
                                </span>) : (t('grade.clickVerseToSelect', 'Click any verse on the page to set it as the actual end verse.'))}
                        </p>
                        {selectedVerseKeyInModal && (<button type="button" onClick={handleConfirmSelection} className="shrink-0 px-4 py-2 rounded-lg bg-primary-600 text-white text-sm font-medium hover:bg-primary-700 shadow-sm">
            ...
            {!isPlanView && singleViewPagesList.length > 0 && !isLoading && !error && (<div className={`flex items-center justify-center gap-4 px-4 py-3 border-b border-gray-200 bg-gray-50 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <button type="button" onClick={goSingleNext} disabled={!canSingleGoNext} className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors" aria-label={t('quran.nextPage', 'Next page')}>
                ...
                : ''}
                    ...
                    <button type="button" onClick={goSinglePrev} disabled={!canSingleGoPrev} className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors" aria-label={t('quran.previousPage', 'Previous page')}>
                        <ChevronRightIcon width={20} height={20} className={isRtl ? '' : 'rotate-180'}/>
                    </button>
                </div>)}

            {/* Plan view: prev/next page navigation (order and arrow direction respect RTL) */}
            {isPlanView && planPages.length > 0 && !isLoading && !isLoadingPlanPages && !error && (<div className={`flex items-center justify-center gap-4 px-4 py-3 border-b border-gray-200 bg-gray-50 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <button type="button" onClick={() => setCurrentPageIndex((i) => Math.min(planPages.length - 1, i + 1))} disabled={currentPageIndex === planPages.length - 1} className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors" aria-label={t('quran.nextPage', 'Next page')}>
                        <ChevronRightIcon width={20} height={20} className={isRtl ? 'rotate-180' : ''}/>
                    </button>
                    ...
                    <button type="button" onClick={() => setCurrentPageIndex((i) => Math.max(0, i - 1))} disabled={currentPageIndex === 0} className="flex items-center justify-center w-10 h-10 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors" aria-label={t('quran.previousPage', 'Previous page')}>
                        <ChevronRightIcon width={20} height={20} className={isRtl ? '' : 'rotate-180'}/>
                    </button>
            ...
            <div className={`relative px-2 sm:px-4 md:px-6 py-2 sm:py-4 ${embedded ? '' : 'overflow-y-auto max-h-[calc(100vh-12rem)]'}`}>
                {isLoading || isLoadingPlanPages ? (<div className="flex items-center justify-center py-20">
                    ...
                    </div>) : (<MushafPage pageLines={pageLines} currentPage={currentPage} wordsDb={wordsDb} surahData={surahData} selectedAyahs={displaySelectedAyahs} isFontLoading={isFontLoading} onWordClick={onVerseKeyClick ? handleVerseKeyClick : onSelectVerseKey ? handleWordClick : undefined}/>)}
            </div>
        </div>);
    if (embedded)
        return content;
