...
import { HALAQA_ACTIVITIES, PLAN_TYPES, PLAN_DIRECTIONS } from '../config';
...
import { loadSurahData, getVerseKeyDisplay, getJuzNumberForVerseKey } from '@/utils/helpers/surahHelper';
/**
 * Create Plan Form Component
 * Creates a plan for a specific student in a halaqa
 */
const CreatePlanForm = ({ halaqaId, students, activities, onSuccess, onCancel }) => {
    ...
    const { control, handleSubmit, formState: { errors }, reset, setValue, watch } = useFormWithValidation({
    ...
    const getSurahName = React.useCallback((surahNumber) => {
    ...
    const formatSegmentVerseInfo = React.useCallback((seg) => {
        if (!seg)
            return '—';
    ...
    const getStartVerseKey = (data) => {
        return data.start_segment_verse_key ?? null;
    };
    const buildPayload = (data, saveOrNot) => {
        const startVerseKey = getStartVerseKey(data);
        if (!startVerseKey || !data.student_ids?.length)
            return null;
        const payload = {
    ...
    const handleConfirmSave = () => {
        ...
        const payload = buildPayload(formValues, 1);
        if (!payload)
            return;
        createPlanMutation.mutate({ halaqaId, data: payload }, {
            onSuccess: () => {
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormSelect name="activity" control={control} label={t('plan.activity', 'Activity')} required options={activityOptions} error={errors.activity?.message}/>
                <FormSelect name="plan_type" control={control} label={t('plan.planType', 'Plan Type')} required options={planTypeOptions} error={errors.plan_type?.message}/>
            </div>
            ...
            <div className={`grid gap-4 ${planType === 'daily_amount' ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
                <FormSelect name="direction" control={control} label={t('plan.direction', 'Direction')} required options={directionOptions} error={errors.direction?.message}/>
                {planType === 'daily_amount' && (<FormInput name="daily_amount" control={control} label={t('plan.dailyAmount', 'Daily Amount')} required type="number" error={errors.daily_amount?.message} min={1} helperText={t('plan.dailyAmountHelperText', 'Enter the daily amount of segments to read. This will be the number of segments read each day.')}/>)}
            </div>

            {/* Students (multi-select) - full width */}
            <SelectRFH name="student_ids" control={control} label={t('plan.students', 'Students')} required isMulti options={studentsOptions} loading={isLoadingStudents} error={errors.student_ids?.message} placeholder={t('plan.selectStudents', 'Select one or more students')}/>
            ...
            <div className="space-y-2">
                <div className="flex items-start flex-col gap-2">
                    ...
                    <Button type="button" onClick={() => setShowMushafSegmentPickerModal(true)} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors" title={t('quran.openMushafViewer', 'Open Mushaf viewer')}>
                        <BookOpenIcon width={18} height={18}/>
                        {t('quran.openMushafViewer', 'Open Mushaf viewer')}
                    </Button>
                </div>
                {(selectedStartSegment || selectedEndSegment) && (<p className="text-sm text-gray-500">
                        {selectedStartSegment && (<span>{t('quran.startSegment', 'Start')}: {formatSegmentVerseInfo(selectedStartSegment)}</span>)}
                        {selectedStartSegment && selectedEndSegment && planType === 'start_end' && ' · '}
                        {selectedEndSegment && planType === 'start_end' && (<span>{t('quran.endSegment', 'End')}: {formatSegmentVerseInfo(selectedEndSegment)}</span>)}
                    </p>)}
            ...
            <MushafSegmentPickerModal isOpen={showMushafSegmentPickerModal} onClose={() => setShowMushafSegmentPickerModal(false)} selectedStartSegment={selectedStartSegment} selectedEndSegment={selectedEndSegment} onSelectStartSegment={setSelectedStartSegment} onSelectEndSegment={setSelectedEndSegment} planType={planType} getSurahName={getSurahName}/>

            {/* Plan Mushaf Viewer Modal — available for all units (segments, juz, surah) after preview */}
            {showPlanMushafViewer && planStartVerseKey && planEndVerseKey && (<MushafPageModal isOpen={showPlanMushafViewer} onClose={() => {
                setShowPlanMushafViewer(false);
                setPlanStartVerseKey(undefined);
                setPlanEndVerseKey(undefined);
            }} startVerseKey={planStartVerseKey} endVerseKey={planEndVerseKey}/>)}

            {/* Plan preview: data from API with save_or_not: 0 — not created yet. Shown for both daily_amount and start_end. */}
            {planPreviewData && (<PlanPreviewCard planPreviewData={planPreviewData} surahData={surahData} currentLang={currentLang} isSaving={createPlanMutation.isPending} onConfirmSave={handleConfirmSave} onBackToEdit={() => setPlanPreviewData(null)} onViewInMushaf={() => {
            ...
            {/* Error Message */}
            {createPlanMutation.error && (<div className="text-red-600 text-sm">
                    {createPlanMutation.error.message || t('plan.createError', 'Error creating plan. Please try again.')}
                </div>)}

            {/* Submit / Cancel */}
            <div className="flex justify-end gap-4">
                {onCancel && (<Button type="button" variant="outline" onClick={onCancel} disabled={createPlanMutation.isPending}>
                        {t('common.cancel', 'Cancel')}
                    </Button>)}
                <Button type="submit" variant="primary" loading={createPlanMutation.isPending} disabled={createPlanMutation.isPending}>
