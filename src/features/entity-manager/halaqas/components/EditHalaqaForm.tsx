...
const EditHalaqaForm: React.FC = () => {
    ...
    useEffect(() => {
        if (evaluationSystemType !== 'رقمي') setValue('custom_total_mark', undefined);
    ...
    const activityOptions = useMemo(() =>
        HALAQA_ACTIVITIES.map((a) => ({ value: a.value, label: t(a.labelKey, a.value) })), [t]);
    const teachingMethodOptions = useMemo(() =>
        HALAQA_TEACHING_METHODS.map((m) => ({ value: m.value, label: t(m.labelKey, m.value) })), [t]);
    const weeklyHolidayOptions = useMemo(() =>
        HALAQA_WEEKLY_HOLIDAYS.map((day) => ({ value: day.value, label: t(day.labelKey, day.label) })), [t]);
    const evaluationSystemOptions = useMemo(() =>
        HALAQA_EVALUATION_SYSTEM_TYPES.map((item) => ({ value: item.value, label: t(item.labelKey, item.label) })), [t]);

    const getErrorMessage = useCallback((message?: string) => (message ? t(message, message) : undefined), [t]);
    ...
    useEffect(() => {
        ...
        const weeklyHoliday = typeof halaqa.weekly_holiday === 'string'
            ? halaqa.weekly_holiday.split(',').map((d: string) => d.trim()).filter(Boolean)
            : [];
