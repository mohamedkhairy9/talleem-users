...
import {
    DEFAULT_HALAQAS_LIST_PARAMS,
    HALAQAS_LIST_PAGINATION
...
export function useHalaqasListState() {
    ...
    const page = parseNum(
        searchParams.get(URL_KEYS.PAGE),
        DEFAULT_HALAQAS_LIST_PARAMS.page ?? HALAQAS_LIST_PAGINATION.DEFAULT_PAGE ?? 1
    );
    const perPage = parseNum(
        searchParams.get(URL_KEYS.PER_PAGE),
        DEFAULT_HALAQAS_LIST_PARAMS.per_page ?? HALAQAS_LIST_PAGINATION.DEFAULT_PER_PAGE ?? 10
    );
    ...
    const setPerPage = useCallback(
        (value: number) => {
            setSearchParams((prev) => {
                ...
                if (value === def) next.delete(URL_KEYS.PER_PAGE);
    ...
    const setPeriod = useCallback(
        (value: string) => {
            setSearchParams((prev) => {
                const next = new URLSearchParams(prev);
                const v = (value ?? '').trim();
                if (v === '') next.delete(URL_KEYS.PERIOD);
                else next.set(URL_KEYS.PERIOD, v);
                next.delete(URL_KEYS.PAGE);
                return next;
            });
        },
    ...
    const setTeachingMethod = useCallback(
        (value: string) => {
            setSearchParams((prev) => {
                const next = new URLSearchParams(prev);
                const v = (value ?? '').trim();
                if (v === '') next.delete(URL_KEYS.TEACHING_METHOD);
                else next.set(URL_KEYS.TEACHING_METHOD, v);
                next.delete(URL_KEYS.PAGE);
                return next;
            });
        },
    ...
    const setFilter = useCallback(
        (key: keyof HalaqasListParams, value: string | number | undefined) => {
            if (key === 'page') setPageSafe((value as number) ?? 1);
            else if (key === 'per_page') setPerPage((value as number) ?? 10);
            else if (key === 'search') setSearch((value as string) ?? '');
            else if (key === 'period') setPeriod((value as string) ?? '');
            else if (key === 'teaching_method') setTeachingMethod((value as string) ?? '');
        },
        [setPageSafe, setPerPage, setSearch, setPeriod, setTeachingMethod]
    );

    const params: HalaqasListParams = useMemo(
        () => ({
            page,
            per_page: perPage,
