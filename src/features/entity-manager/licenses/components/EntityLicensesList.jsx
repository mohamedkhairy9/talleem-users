...
const EntityLicensesList = () => {
    ...
    const columns = useMemo(() => createEntityLicensesListColumns({ t, getLocalizedText }), [t, getLocalizedText]);
    if (error) {
    ...
    return (<div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <div className="flex flex-1 flex-col overflow-hidden md:hidden min-h-[280px] bg-white rounded-lg">
                <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden">
                    <EntityLicensesListMobile list={list} isLoading={isLoading} hasError={!!error} errorMessage={error ? t('licenses.loadError', 'Error loading licenses.') : undefined} emptyMessage={t('licenses.noLicenses', 'No licenses found.')} getLocalizedText={getLocalizedText}/>
                </div>
            </div>
