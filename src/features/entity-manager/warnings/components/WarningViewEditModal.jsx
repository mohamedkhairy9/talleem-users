 ...
 */
const WarningViewEditModal = ({ isOpen, warningId, onClose, mode: initialMode = 'view' }) => {
    ...
    const warning = warningData?.data || warningData;
    // Extract entity data
    ...
    return (<div className="fixed inset-0 z-[60] overflow-y-auto">
        ...
        />
                                    ...
                                    {warningType === 'student' && (<SelectRFH name="student_id" label={t('warning.student', 'Student')} control={control} error={errors.student_id?.message} isAsync={true} loadOptions={studentsLoader} defaultOptions={true} cacheOptions={true} disabled={isTargetFieldDisabled} placeholder={t('warning.searchStudent')}/>)}

                                    {/* Conditional: Teacher ID */}
                                    {warningType === 'teacher' && (<SelectRFH name="teacher_id" label={t('warning.teacher', 'Teacher')} control={control} error={errors.teacher_id?.message} isAsync={true} loadOptions={teachersLoader} defaultOptions={true} cacheOptions={true} disabled={isTargetFieldDisabled} placeholder={t('warning.searchTeacher')}/>)}
